const puzzleData = {
	PUZZLE_ID: "02",
	SIZE: 10,
	MIN_WORD_LENGTH: 3,

	BLACK_FIELDS: [
		[0, 0],
		[0, 2],
		[0, 7],
		[0, 9],
		[1, 4],
		[1, 5],

		[3, 2],
		[3, 7],
		[4, 1],
		[4, 8],
		[5, 1],
		[5, 8],
		[6, 2],
		[6, 7],

		[8, 4],
		[8, 5],

		[9, 0],
		[9, 2],
		[9, 7],
		[9, 9],
	],
	// NUMBER_FIELDS: [
	// 	[0, 0],
	// 	[0, 3],
	// 	[0, 6],

	// 	[1, 1],
	// 	[1, 8],

	// 	[2, 4],
	// 	[2, 5],

	// 	[3, 0],
	// 	[3, 3],
	// 	[3, 6],
	// 	[3, 9],

	// 	[4, 2],
	// 	[5, 2],

	// 	[6, 0],
	// 	[6, 3],
	// 	[6, 8],

	// 	[8, 3],
	// 	[8, 6],

	// 	[9, 0],
	// ],
	CLUES: {
		WAAGERECHT: {
			2: {
				c: "Im Match nur Set-Etappe",
				l: "4",
				s: "GAME",
			},
			5: {
				c: "Beim Gebäck stehts für doppelt, beim Licht für halb",
				l: "4",
				s: "ZWIE",
			},
			6: {
				c: "Schäuble wollte die in schwarz",
				l: "4",
				s: "NULL",
			},
			8: {
				c: "Steinerne Motivwahl bei der Hautverziehrung? Flotte Dekoration als direkter Nachfolger der Höhlenmalerei!",
				l: "10",
				s: "WANDTATTOO",
			},
			11: {
				c: "Vorm End wirds ganz bezaubernd, der nervige",
				l: "4",
				s: "REIZ",
			},
			12: {
				c: "Lautstarke Zwischenbemerkung, hat alles was Furien haben",
				l: "6",
				s: "EINRUF",
			},
			13: {
				c: "In zwei Wörtern: Beschreibt höher gelegenen Bereich, in einem: Benjamin mit Dauerkarte",
				l: "2,4",
				s: "ABOBEN",
			},
			15: {
				c: "Synonyme de marron",
				l: "4",
				s: "BRUN",
			},
			17: {
				c: "Berberitze Spitz-Name aus verdrehtem Neidspross",
				l: "10",
				s: "SPIESSDORN",
			},
			18: {
				c: "Schweinischer Aufruf zum Schlammbad? Heimatort von Schwalbe, Simme und Co",
				l: "4",
				s: "SUHL",
			},
			19: {
				c: "Unendlicher Kern der Heddewigsnelke",
				l: "4",
				s: "EWIG",
			},
			20: {
				c: "Steht geschrieben: Nach Plain, Tele, Blind",
				l: "4",
				s: "TEXT",
			},
		},
		SENKRECHT: {
			1: {
				c: "Wird aufgedreht nach dem Aufstehen und vor dem Blick in den Spiegel",
				l: "4",
				s: "SWAG",
			},
			2: {
				c: "Attribut aus der Trickkiste für System oder Verteidiger",
				l: "10",
				s: "GEDRIBBELT",
			},
			3: {
				c: 'Sprichwörtlich: "Die Wunde ... den Finger, der Gedanke den Geist"',
				l: "10",
				s: "ENTZUENDET",
			},
			4: {
				c: "Passt zum Walzer, Markt und Biss",
				l: "4",
				s: "FLOH",
			},
			5: {
				c: "12 Zoll mit dem Körper gemessen",
				l: "4,4",
				s: "ZWEIFUSS",
			},
			7: {
				c: "Ein verliebter Robert Smith lieferte den 20 waagerecht dazu",
				l: "8",
				s: "LOVESONG",
			},
			9: {
				c: "Die klingen höchstens im opera house",
				l: "6",
				s: "TENORS",
			},
			10: {
				c: "Überflieger im Überfliegen",
				l: "6",
				s: "AIRBUS",
			},
			14: {
				c: "Ein Hauch von Weg",
				l: "4",
				s: "SPUR",
			},
			16: {
				c: "Ausgezeichnete bekommen den weiter westlich",
				l: "4",
				s: "PRIX",
			},
		},
	},
};
